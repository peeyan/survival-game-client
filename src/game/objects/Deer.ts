...
export default class Deer extends Phaser.Physics.Arcade.Sprite {
  ...
  private readonly fleeRange = 380;
  ...
  update(player: Phaser.GameObjects.Sprite, _gs: GameState): void {
    ...
    if (dist < this.fleeRange * detectMult) this.isFleeing = true;
    else if (dist > this.fleeRange * 1.6) this.isFleeing = false;
    ...
    if (this.isFleeing) {
      ...
      if (this.wanderTimer <= 0) {
        this.wanderAngle += Phaser.Math.FloatBetween(-0.6, 0.6);
        this.wanderTimer = Phaser.Math.Between(1200, 3500);
      }
      body.setVelocity(Math.cos(this.wanderAngle) * 45, Math.sin(this.wanderAngle) * 45);
    }
